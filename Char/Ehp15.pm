...
BEGIN {
    ...
    if ($] < 5.006) {
        ...
        if ($^O ne 'MacOS') {
            *CORE::GLOBAL::binmode =
            *CORE::GLOBAL::binmode = \&Char::Ehp15::binmode;
        }
        *CORE::GLOBAL::open    =
        *CORE::GLOBAL::open    = \&Char::Ehp15::open;
    }
}
...
if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
    if ($ENV{'ComSpec'} =~ / (?: COMMAND\.COM | CMD\.EXE ) \z /oxmsi) {
        ...
        for (@ARGV) {
            ...
            elsif (m/\A (?:$q_char)*? [*?] /oxms and (my @glob = Char::Ehp15::glob($_))) {
                push @argv, @glob;
            }
            else {
                push @argv, $_;
...
#
{
    ...
    @Char::Ehp15::m_matched = (qr/(?{Char::Ehp15::m_matched})/);
    @Char::Ehp15::s_matched = (qr/(?{Char::Ehp15::s_matched})/);
}
...
sub _octets {
    ...
    if ($length == 1) {
        ...
        if (((caller(1))[3] ne 'Char::Ehp15::_octets') and ($modifier =~ m/i/oxms)) {
            if ($a == $z) {
...
sub Char::Ehp15::T(;*@) {
    ...
    local $_ = shift if @_;
    croak 'Too many arguments for -T (Char::Ehp15::T)' if @_ and not wantarray;
    my $T = 1;
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $uc = Char::Ehp15::uc($char)) ne $char) {
                $pattern .= $uc;
            }
            else {
        ...
        my $matchsub = sub { Char::Ehp15::uc($_[0]) =~ m{\A $pattern \z}xms };
...
=item Split string
  ...
  $string on every match. If the pattern doesn't match at all, Char::Ehp15::split returns
...
=item Chop string
  ...
  The Char::Ehp15::chop operator is used primarily to remove the newline from the end of
  an input record but is more efficient than s/\n$//. If no argument is given, the
  function chops the $_ variable.

=item Index string

  $pos = Char::Ehp15::index($string,$substr,$position);
  $pos = Char::Ehp15::index($string,$substr);
  ...
  the Char::Ehp15::index function returns -1.

=item Reverse index string

  $pos = Char::Ehp15::rindex($string,$substr,$position);
  $pos = Char::Ehp15::rindex($string,$substr);

  Works just like Char::Ehp15::index except that it returns the position of the last
...
=item Make character
  ...
  set. For example, Char::Ehp15::chr(65) is "A" in either ASCII or HP-15, and
  Char::Ehp15::chr(0x82a0) is a HP-15 HIRAGANA LETTER A. For the reverse of Char::Ehp15::chr,
  use Char::HP15::ord.

=item File test operator -X

  A file test operator is an unary operator that tests a pathname or a filehandle.
  If $string is omitted, it uses $_ by function Char::Ehp15::r_.
  The following functions function when the pathname ends with chr(0x5C) on MSWin32.

  $test = Char::Ehp15::r $string;
  $test = Char::Ehp15::r_;
  ...
  ------------------------------------------------------------------------------
  Char::Ehp15::r(*), Char::Ehp15::r_()   File is readable by effective uid/gid
  Char::Ehp15::w(*), Char::Ehp15::w_()   File is writable by effective uid/gid
  Char::Ehp15::x(*), Char::Ehp15::x_()   File is executable by effective uid/gid
  Char::Ehp15::o(*), Char::Ehp15::o_()   File is owned by effective uid
  Char::Ehp15::R(*), Char::Ehp15::R_()   File is readable by real uid/gid
  Char::Ehp15::W(*), Char::Ehp15::W_()   File is writable by real uid/gid
  Char::Ehp15::X(*), Char::Ehp15::X_()   File is executable by real uid/gid
  Char::Ehp15::O(*), Char::Ehp15::O_()   File is owned by real uid
  Char::Ehp15::e(*), Char::Ehp15::e_()   File exists
  Char::Ehp15::z(*), Char::Ehp15::z_()   File has zero size
  Char::Ehp15::f(*), Char::Ehp15::f_()   File is a plain file
  Char::Ehp15::d(*), Char::Ehp15::d_()   File is a directory
  Char::Ehp15::l(*), Char::Ehp15::l_()   File is a symbolic link
  Char::Ehp15::p(*), Char::Ehp15::p_()   File is a named pipe (FIFO)
  Char::Ehp15::S(*), Char::Ehp15::S_()   File is a socket
  Char::Ehp15::b(*), Char::Ehp15::b_()   File is a block special file
  Char::Ehp15::c(*), Char::Ehp15::c_()   File is a character special file
  Char::Ehp15::u(*), Char::Ehp15::u_()   File has setuid bit set
  Char::Ehp15::g(*), Char::Ehp15::g_()   File has setgid bit set
  Char::Ehp15::k(*), Char::Ehp15::k_()   File has sticky bit set
  ------------------------------------------------------------------------------
  ...
  a filehandle. Both Char::Ehp15::T and Char::Ehp15::B deosn't work when given the special
  filehandle consisting of a solitary underline.

  Function and Prototype     Meaning
  ------------------------------------------------------------------------------
  Char::Ehp15::T(*), Char::Ehp15::T_()   File is a text file
  Char::Ehp15::B(*), Char::Ehp15::B_()   File is a binary file (opposite of -T)
  ------------------------------------------------------------------------------

  Returns useful value if successful, or undef unless successful.

  $value = Char::Ehp15::s $string;
  $value = Char::Ehp15::s_;

  Function and Prototype     Meaning
  ------------------------------------------------------------------------------
  Char::Ehp15::s(*), Char::Ehp15::s_()   File has nonzero size (returns size)
  Char::Ehp15::M(*), Char::Ehp15::M_()   Age of file (at startup) in days since modification
  Char::Ehp15::A(*), Char::Ehp15::A_()   Age of file (at startup) in days since last access
  Char::Ehp15::C(*), Char::Ehp15::C_()   Age of file (at startup) in days since inode change
  ------------------------------------------------------------------------------
...
=item Statistics about file
  ...
  ($dev,$ino,$mode,$nlink,$uid,$gid,$rdev,$size,
      $atime,$mtime,$ctime,$blksize,$blocks) = Char::Ehp15::stat($file);
...
=item require file
  ...
  If the argument is a string, Char::Ehp15::require loads and executes the Perl code found in
  the separate file whose name is given by the string. This is similar to performing a
  Char::Ehp15::do on a file, except that Char::Ehp15::require checks to see whether the library
  ...
  duplicate compilation.) Like its cousins Char::Ehp15::do and use, Char::Ehp15::require knows how
...
=item binary mode (Perl5.6 emulation on perl5.005)
  ...
  Char::Ehp15::binmode(FILEHANDLE, $disciplines);
  Char::Ehp15::binmode(FILEHANDLE);
  Char::Ehp15::binmode($filehandle, $disciplines);
  Char::Ehp15::binmode($filehandle);
...
=item open file (Perl5.6 emulation on perl5.005)
  ...
  $rc = Char::Ehp15::open(FILEHANDLE, $mode, $expr);
  $rc = Char::Ehp15::open(FILEHANDLE, $expr);
  $rc = Char::Ehp15::open(FILEHANDLE);
  $rc = Char::Ehp15::open(my $filehandle, $mode, $expr);
  $rc = Char::Ehp15::open(my $filehandle, $expr);
  $rc = Char::Ehp15::open(my $filehandle);
  ...
  * three arguments
  ...
  {
      my $fh;                          # (uninitialized)
      Char::Ehp15::open($fh, ">logfile")     # $fh is autovivified
  ...
  Char::Ehp15::open my $fh, ">logfile" or die ...
  ...
  Char::Ehp15::open(LOG, ">", "logfile") or die "Can't create logfile: $!";
  ...
  $path =~ s#^([ ])#./$1#;
  Char::Ehp15::open (FH, "< $path\0") or die "can't open $path: $!";
  ...
  Char::Ehp15::open(FH, "<", $path) or die "can't open $path: $!";
  ...
  Char::Ehp15::open(FH, "<:raw", $path) or die "can't open $path: $!";
  ...
  Char::Ehp15::open(FH, "<:crlf:encoding(Char::HP15)", $path) or die "can't open $path: $!";
