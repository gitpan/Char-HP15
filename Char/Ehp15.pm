...
sub _parse_path {
    ...
    my @subpath = ();
    while ($path =~ m{
        ((?: [\x80-\xA0\xE0-\xFE][\x00-\xFF]|[^\x80-\xA0\xE0-\xFE/\\] )+?) [/\\] }oxmsg
    ) {
        push @subpath, $1;
    }
...
=item Make character
  ...
  set. For example, Char::Ehp15::chr(65) is "A" in either ASCII or HP-15, not Unicode,
  and Char::Ehp15::chr(0x82a0) is a HP-15 HIRAGANA LETTER A. For the reverse of
  Char::Ehp15::chr, use Char::HP15::ord.
...
=item File test operator -X
  ...
  Function and Prototype     Meaning
  ------------------------------------------------------------------------------
  Char::Ehp15::s(*), Char::Ehp15::s_()   File has nonzero size (returns size in bytes)
