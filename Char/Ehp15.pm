...
# regexp of character
BEGIN { CORE::eval q{ use vars qw($your_char) } } $your_char = q{[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[\x00-\xFF]};
...
if (($] >= 5.010001) or
    ...
    # other encoding
        ...
        ${Char::Ehp15::q_char_SADAHIRO_Tomoyuki_2002_01_17} = qr{.*?[$sbcs](?:$tbcs_1st[^$sbcs]{2})*?}oxms;
    }

    ${Char::Ehp15::anchor_SADAHIRO_Tomoyuki_2002_01_17} =
    qr{\G(?(?=.{0,32766}\z)(?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[^\x80-\xA0\xE0-\xFE])*?|(?(?=[$sbcs]+\z).*?|(?:${Char::Ehp15::q_char_SADAHIRO_Tomoyuki_2002_01_17})))}oxms;
#   qr{
#      \G # (1), (2)
#        (? # (3)
#          (?=.{0,32766}\z) # (4)
...
#                                                                                      (?(?=[$sbcs]+\z) # (6)
#                                                                                                      .*?| #(7)
#                                                                                                          (?:${Char::Ehp15::q_char_SADAHIRO_Tomoyuki_2002_01_17}) # (8)
#                                                                                                                                                           ))}oxms;
    ...
    if (((('A' x 32768).'B') !~ / ${Char::Ehp15::anchor}                              B /oxms) and
        ...
        undef ${Char::Ehp15::q_char_SADAHIRO_Tomoyuki_2002_01_17};
    }
}
...
sub Char::Ehp15::split(;$$$) {
    ...
    if (CORE::length($string) > 32766) {
        ...
        elsif (defined ${Char::Ehp15::q_char_SADAHIRO_Tomoyuki_2002_01_17}) {
            $q_char = ${Char::Ehp15::q_char_SADAHIRO_Tomoyuki_2002_01_17};
        }
    }
...
sub Char::Ehp15::filetest {
    ...
    unless (CORE::eval qq{Char::Ehp15::$filetest(\$file)}) {
        return '';
    }
...
sub Char::Ehp15::filetest_ {
    ...
    unless (CORE::eval qq{Char::Ehp15::${filetest}_}) {
        return '';
    }
    for my $filetest (CORE::reverse @_) {
        unless (CORE::eval qq{ $filetest _ }) {
...
sub my_home {
    ...
    if (defined $home and ! Char::Ehp15::d($home)) {
        $home = undef;
    }
    return $home;
}
        ...
        # rejected idea ...
            ...
            if (($] =~ /^5\.006/oxms)                     and
                ...
                *{'Dyna'.'Loader::dl_install_xsub'}{'CODE'}->('_SetCurrentDirectoryA', unpack('L', pack 'P', $x86));
                ...
                if (Char::Ehp15::fc($chdir) eq Char::Ehp15::fc($dir)) {
                    return 1;
                }
                else {
                    return 0;
...
#   long filename form.
        ...
        if (qx{ver} =~ /\b(?:Windows NT|Windows 2000)\b/oms) {
            ...
            for my $dirx (sort { CORE::length($a) <=> CORE::length($b) } @dirx) {
                if (Char::Ehp15::fc(CORE::substr $dirx,-CORE::length($subdir[-1]),CORE::length($subdir[-1])) eq Char::Ehp15::fc($subdir[-1])) {
        ...
        elsif (Char::Ehp15::fc($shortdir) eq Char::Ehp15::fc($dir)) {
            return 0;
        }
        return CORE::chdir $shortdir;
    }
...
sub Char::HP15::escape(;$) {
    ...
    while (not /\G \z/oxgc) { # member
        $e_script .= Char::HP15::escape_token();
    }
...
# subroutines of package Char::Ehp15
    ...
    elsif (/\G \b Char::HP15::eval       (?= \s* \{ )                /oxgc) { $slash = 'm//'; return 'eval';              }
    elsif (/\G \b Char::HP15::eval    \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'eval Char::HP15::escape'; }
...
# subroutines of package Char::Ehp15
        ...
        elsif ($string =~ /\G \b Char::HP15::eval       (?= \s* \{ )                /oxgc) { $e_string .= 'eval';              $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::HP15::eval \b                                /oxgc) { $e_string .= 'eval Char::HP15::escape'; $slash = 'm//'; }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr with multibyte anchoring
            ...
            $sub = sprintf(
                #                              1                                              2   3                                 4   5
                q<CORE::eval{local $Char::HP15::re_t=%s; local $Char::HP15::re_a=''; while($Char::HP15::re_t =~ %s){%s local $^W=0; local $Char::HP15::re_r=%s; %s$Char::HP15::re_t="$Char::HP15::re_a${1}$Char::HP15::re_r$'"; pos($Char::HP15::re_t)=length "$Char::HP15::re_a${1}$Char::HP15::re_r"; $Char::HP15::re_a=substr($Char::HP15::re_t,0,pos($Char::HP15::re_t)); } return $Char::HP15::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Char::HP15::re_r=CORE::eval $Char::HP15::re_r; ' x $e_modifier,            #  5
            );
        }
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                              1                        2   3                                 4   5
                q<CORE::eval{local $Char::HP15::re_t=%s; while($Char::HP15::re_t =~ %s){%s local $^W=0; local $Char::HP15::re_r=%s; %s$Char::HP15::re_t="$`$Char::HP15::re_r$'"; pos($Char::HP15::re_t)=length "$`$Char::HP15::re_r"; } return $Char::HP15::re_t}>,
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2                3                                 4   5  6                    7
                q<(%s =~ %s) ? CORE::eval{%s local $^W=0; local $Char::HP15::re_r=%s; %s"%s$Char::HP15::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g with multibyte anchoring
            ...
            $sub = sprintf(
                ...
                q<CORE::eval{local $Char::HP15::re_n=0; local $Char::HP15::re_a=''; while(%s =~ %s){%s local $^W=0; local $Char::HP15::re_r=%s; %s%s="$Char::HP15::re_a${1}$Char::HP15::re_r$'"; pos(%s)=length "$Char::HP15::re_a${1}$Char::HP15::re_r"; $Char::HP15::re_a=substr(%s,0,pos(%s)); $Char::HP15::re_n++} return %s$Char::HP15::re_n}>,
...
# (and so on)
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                       1     2   3                                 4   5 6                         7                                                 8
                q<CORE::eval{local $Char::HP15::re_n=0; while(%s =~ %s){%s local $^W=0; local $Char::HP15::re_r=%s; %s%s="$`$Char::HP15::re_r$'"; pos(%s)=length "$`$Char::HP15::re_r"; $Char::HP15::re_n++} return %s$Char::HP15::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3                4                                 5   6 7   8
                q<(%s%s%s) ? CORE::eval{%s local $^W=0; local $Char::HP15::re_r=%s; %s%s="%s$Char::HP15::re_r$'"; 1 } : undef> :

                #  1 2 3                    4                                 5   6 7   8
                q<(%s%s%s) ? 1 : CORE::eval{%s local $^W=0; local $Char::HP15::re_r=%s; %s%s="%s$Char::HP15::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Char::HP15::re_r=CORE::eval $Char::HP15::re_r; ' x $e_modifier,            #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
This module has run-time routines for use Char::HP15 software automatically, you
do not have to use.
...
=item Chop string
  ...
  character chopped. The Char::Ehp15::chop subroutine is used primary to remove the newline
...
=item Reverse index string
  ...
  This subroutine works just like Char::Ehp15::index except that it returns the byte
...
=item Lower case string
  ...
  You can use the Char::Ehp15::fc subroutine for case-insensitive comparisons via Char::HP15
  software.
...
=item Fold case string
  ...
  New to Char::HP15 software, this subroutine returns the full Unicode-like casefold of
...
=item File test subroutine Char::Ehp15::X
...
=item Filename expansion (globbing)
  ...
  The Char::Ehp15::glob subroutine grandfathers the use of whitespace to separate multiple
...
=item Statistics about file
  ...
  the last Char::Ehp15::stat, Char::Ehp15::lstat, or Char::Ehp15::stat-based file test subroutine
