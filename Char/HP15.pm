...
# while (<WILDCARD>) --- glob
    ...
    elsif (/\G \b while \s* \( \s* < ((?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[^>\0\a\e\f\n\r\t])+?) > \s* \) \b /oxgc) {
...
# <WILDCARD> --- glob
    ...
    elsif (/\G < ((?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[^>\0\a\e\f\n\r\t])+?) > /oxgc) {
...
sub classic_character_class {
    ...
    return {
        '.'  => ($modifier =~ /s/) ?
                '(?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[\x00-\xFF])' :
                '(?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[^\x0A])',
        '\D' => '(?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[^0-9])',
        '\S' => '(?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[^\x09\x0A\x0C\x0D\x20])',
        '\W' => '(?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[^0-9A-Z_a-z])',
        '\d' => '[0-9]',
                 # \t  \n  \f  \r space
        ...
        '\H' => '(?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[^\x09\x20])',
        '\V' => '(?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[^\x0C\x0A\x0D])',
        '\h' => '[\x09\x20]',
        '\v' => '[\x0C\x0A\x0D]',
...
sub e_use_noimport {
    ...
    for my $realfilename (_realfilename($expr)) {
        ...
        if (CORE::open($fh, $realfilename)) {
            ...
            if ($script =~ m/^ \s* use \s+ Char::HP15 \s* ([^;]*) ; \s* \n? $/oxms) {
...
and the cellular phone. This software treats HP-15 directly. Therefor there is
not UTF8 flag.
...
The character classes are redefined as follows to backward compatibility.
  ...
  ---------------------------------------------------------------------------
   .          (?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[^\x0A])
              (?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[\x00-\xFF]) (/s modifier)
  \d          [0-9]
  \s          [\x09\x0A\x0C\x0D\x20]
  \w          [0-9A-Z_a-z]
  \D          (?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[^0-9])
...
=head1 Perl5.6 Emulation On perl5.005
  ...
  --------------------------------------------------------------------
  Before          After                  in BEGIN { } of Char/Ehp15.pm
  --------------------------------------------------------------------
  binmode(...);   Char::Ehp15::binmode(...);   *CORE::GLOBAL::binmode = ...
  open(...);      Char::Ehp15::open(...);      *CORE::GLOBAL::open    = ...
  --------------------------------------------------------------------
...
=item * binmode (Perl5.6 emulation on perl5.005)
  ...
  * two arguments
  ...
  If you are using perl5.005 other than MacPerl, Char::HP15 software emulate perl5.6's
...
=item * open (Perl5.6 emulation on perl5.005)
  ...
  * three arguments
  ...
  If you are using perl5.005, Char::HP15 software emulate perl5.6's open function.
  ...
  open(FH, "<:crlf:encoding(Char::HP15)", $path) or die "can't open $path: $!";
