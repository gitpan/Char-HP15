...
######################################################################
#
# Char::HP15 - Source code filter to escape HP-15 script
#
# http://search.cpan.org/dist/Char-Char::HP15/
#
...
my $qq_substr  = qr{(?: Char::HP15::substr | CORE::substr | substr ) \( $qq_paren \)
                 }xms;
...
my $matched     = '';
my $s_matched   = '';
$matched        = q{$Char::Ehp15::matched};
$s_matched      = q{ Char::Ehp15::s_matched();};
...
my $function_ord;         # ord()   to ord()   or Char::HP15::ord()
my $function_ord_;        # ord     to ord     or Char::HP15::ord_
my $function_reverse;     # reverse to reverse or Char::HP15::reverse
my $function_getc;        # getc    to getc    or Char::HP15::getc
...
END
    ...
    print Char::HP15::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (Char::Ehp15::e("$filename.e")) {
        if (exists $ENV{'SJIS_DEBUG'}) {
            Char::Ehp15::unlink "$filename.e";
        }
        elsif (Char::Ehp15::z("$filename.e")) {
            Char::Ehp15::unlink "$filename.e";
        }
        else {
            my $e_mtime   = (Char::Ehp15::stat("$filename.e"))[9];
            my $mtime     = (Char::Ehp15::stat($filename))[9];
            my $__mtime__ = (Char::Ehp15::stat(__FILE__))[9];
    ...
    if (not Char::Ehp15::e("$filename.e")) {
        my $fh = gensym();

        if (eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
        }
        else {
            Char::Ehp15::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e";
        }
        ...
        my $e_script = Char::HP15::escape_script($filename);
        print {$fh} $e_script;

        my $mode = (Char::Ehp15::stat($filename))[2] & 0777;
    ...
    my $fh = gensym();
    Char::Ehp15::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e";
...
sub Char::HP15::escape_script {
    ...
    # read HP-15 script
    my $fh = gensym();
    Char::Ehp15::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script";

    if (/^ use Char::Ehp15(?:\s+[0-9\.]*)?\s*; $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Char::Ehp15 %s;\n", $Char::HP15::VERSION); # require run-time routines version

        # use Char::HP15 version qw(ord reverse getc);
        $function_ord     = 'ord';
        $function_ord_    = 'ord';
        ...
        if (s/^ \s* use \s+ Char::HP15 \s* ([^\x80-\xA0\xE0-\xFE;]*) ; \s* \n? $//oxms) {
            ...
            if ($list =~ s/\A ([0-9]+\.[0-9]+) \.0 \s* //oxms) {
                my $version = $1;
                if ($version ne $Char::HP15::VERSION) {
                    my @file = grep -e, map {qq{$_/Char/HP15.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Char/HP15.pm $version, but @{[__FILE__]} is version $Char::HP15::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Char/Ehp15.pm $version, but \\\$Char::Ehp15::VERSION is \$Char::Ehp15::VERSION" if \$Char::Ehp15::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ([0-9]+(?:\.[0-9]*)) \s* //oxms) {
                my $version = $1;
                if ($version > $Char::HP15::VERSION) {
                    die "Script $0 required Char/HP15.pm $version, but @{[__FILE__]} is only version $Char::HP15::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Char::HP15::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Char::HP15::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Char::HP15::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Char::HP15::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
# $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Char::Ehp15::PREMATCH()
    elsif (/\G ( \$` | \$\{`\} | \$ \s* PREMATCH  \b | \$ \s* \{ \s* PREMATCH  \s* \} | \$ \s* \{\^PREMATCH\}  ) /oxmsgc) {
        $slash = 'div';
        return q{Char::Ehp15::PREMATCH()};
    }

# $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Char::Ehp15::MATCH()
    elsif (/\G ( \$& | \$\{&\} | \$ \s* MATCH     \b | \$ \s* \{ \s* MATCH     \s* \} | \$ \s* \{\^MATCH\}     ) /oxmsgc) {
        $slash = 'div';
        return q{Char::Ehp15::MATCH()};
    }

# $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Char::Ehp15::POSTMATCH()
    elsif (/\G (                 \$ \s* POSTMATCH \b | \$ \s* \{ \s* POSTMATCH \s* \} | \$ \s* \{\^POSTMATCH\} ) /oxmsgc) {
        $slash = 'div';
        return q{Char::Ehp15::POSTMATCH()};
    }
...
# while (<WILDCARD>) --- glob
    ...
    elsif (/\G \b while \s* \( \s* < ((?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[^\x80-\xA0\xE0-\xFE>\0\a\e\f\n\r\t])+?) > \s* \) \b /oxgc) {
        return 'while ($_ = Char::Ehp15::glob("' . $1 . '"))';
    }

# while (glob)
    elsif (/\G \b while \s* \( \s* glob \s* \) /oxgc) {
        return 'while ($_ = Char::Ehp15::glob_)';
    }

# while (glob(WILDCARD))
    elsif (/\G \b while \s* \( \s* glob \b /oxgc) {
        return 'while ($_ = Char::Ehp15::glob';
    }
...
# functions of package Char::Ehp15
    elsif (/\G \b (CORE:: | ->[ ]* (?: atan2 | [a-z]{2,})) \b  /oxgc) { $slash = 'm//'; return $1;                    }
    elsif (/\G \b bytes::substr \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'substr';              }
    elsif (/\G \b chop \b          (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::Ehp15::chop';         }
    elsif (/\G \b bytes::index \b  (?! \s* => )                /oxgc) { $slash = 'm//'; return 'index';               }
    elsif (/\G \b Char::HP15::index \b   (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::HP15::index';         }
    elsif (/\G \b index \b         (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::Ehp15::index';        }
    elsif (/\G \b bytes::rindex \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'rindex';              }
    elsif (/\G \b Char::HP15::rindex \b  (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::HP15::rindex';        }
    elsif (/\G \b rindex \b        (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::Ehp15::rindex';       }
    elsif (/\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Ehp15::lc';           }
    elsif (/\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Ehp15::lcfirst';      }
    elsif (/\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Ehp15::uc';           }
    elsif (/\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Ehp15::ucfirst';      }
    elsif (/\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Ehp15::fc';           }
    ...
    # (and so on)
                                                         ...
                                                         \s* (\") ((?:$qq_char)+?)             (\") /oxgc) { $slash = 'm//'; return "Char::Ehp15::filetest(qw($1)," . e_qq('',  $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\#) ((?:$qq_char)+?)             (\#) /oxgc) { $slash = 'm//'; return "Char::Ehp15::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\() ((?:$qq_paren)+?)            (\)) /oxgc) { $slash = 'm//'; return "Char::Ehp15::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\{) ((?:$qq_brace)+?)            (\}) /oxgc) { $slash = 'm//'; return "Char::Ehp15::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\[) ((?:$qq_bracket)+?)          (\]) /oxgc) { $slash = 'm//'; return "Char::Ehp15::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\<) ((?:$qq_angle)+?)            (\>) /oxgc) { $slash = 'm//'; return "Char::Ehp15::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc) { $slash = 'm//'; return "Char::Ehp15::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }

    elsif (/\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+)
                                                         \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc) { $slash = 'm//'; return "Char::Ehp15::filetest(qw($1)," . e_q ('',  $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\#) ((?:\\\#|\\\\|$q_char)+?)    (\#) /oxgc) { $slash = 'm//'; return "Char::Ehp15::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\() ((?:\\\)|\\\\|$q_paren)+?)   (\)) /oxgc) { $slash = 'm//'; return "Char::Ehp15::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\{) ((?:\\\}|\\\\|$q_brace)+?)   (\}) /oxgc) { $slash = 'm//'; return "Char::Ehp15::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\[) ((?:\\\]|\\\\|$q_bracket)+?) (\]) /oxgc) { $slash = 'm//'; return "Char::Ehp15::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\<) ((?:\\\>|\\\\|$q_angle)+?)   (\>) /oxgc) { $slash = 'm//'; return "Char::Ehp15::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc) { $slash = 'm//'; return "Char::Ehp15::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }

    elsif (/\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+) (\$ \w+(?: ::\w+)* (?: (?: ->)? (?: \( (?:$qq_paren)*? \) | \{ (?:$qq_brace)+? \} | \[ (?:$qq_bracket)+? \] ) )*) /oxgc)
                                                                                                           { $slash = 'm//'; return "Char::Ehp15::filetest(qw($1),$2)"; }
    elsif (/\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+) \( ((?:$qq_paren)*?) \) /oxgc)
                                                                                                           { $slash = 'm//'; return "Char::Ehp15::filetest(qw($1),$2)"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) (?= [a-z]+) /oxgc)                                   { $slash = 'm//'; return "Char::Ehp15::filetest qw($1),";    }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) (\w+) /oxgc)                                         { $slash = 'm//'; return "Char::Ehp15::filetest(qw($1),$2)"; }

    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\") ((?:$qq_char)+?)             (\") /oxgc)    { $slash = 'm//'; return "Char::Ehp15::$1(" . e_qq('',  $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\#) ((?:$qq_char)+?)             (\#) /oxgc)    { $slash = 'm//'; return "Char::Ehp15::$1(" . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\() ((?:$qq_paren)+?)            (\)) /oxgc)    { $slash = 'm//'; return "Char::Ehp15::$1(" . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\{) ((?:$qq_brace)+?)            (\}) /oxgc)    { $slash = 'm//'; return "Char::Ehp15::$1(" . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\[) ((?:$qq_bracket)+?)          (\]) /oxgc)    { $slash = 'm//'; return "Char::Ehp15::$1(" . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\<) ((?:$qq_angle)+?)            (\>) /oxgc)    { $slash = 'm//'; return "Char::Ehp15::$1(" . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc)    { $slash = 'm//'; return "Char::Ehp15::$1(" . e_qq('qq',$2,$4,$3) . ")"; }

    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc)    { $slash = 'm//'; return "Char::Ehp15::$1(" . e_q ('',  $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\#) ((?:\\\#|\\\\|$q_char)+?)    (\#) /oxgc)    { $slash = 'm//'; return "Char::Ehp15::$1(" . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\() ((?:\\\)|\\\\|$q_paren)+?)   (\)) /oxgc)    { $slash = 'm//'; return "Char::Ehp15::$1(" . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\{) ((?:\\\}|\\\\|$q_brace)+?)   (\}) /oxgc)    { $slash = 'm//'; return "Char::Ehp15::$1(" . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\[) ((?:\\\]|\\\\|$q_bracket)+?) (\]) /oxgc)    { $slash = 'm//'; return "Char::Ehp15::$1(" . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\<) ((?:\\\>|\\\\|$q_angle)+?)   (\>) /oxgc)    { $slash = 'm//'; return "Char::Ehp15::$1(" . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc)    { $slash = 'm//'; return "Char::Ehp15::$1(" . e_q ('q', $2,$4,$3) . ")"; }

    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s* (\$ \w+(?: ::\w+)* (?: (?: ->)? (?: \( (?:$qq_paren)*? \) | \{ (?:$qq_brace)+? \} | \[ (?:$qq_bracket)+? \] ) )*) /oxgc)
                                                                                                           { $slash = 'm//'; return "Char::Ehp15::$1($2)";      }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s* \( ((?:$qq_paren)*?) \) /oxgc)                          { $slash = 'm//'; return "Char::Ehp15::$1($2)";      }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) (?= \s+ [a-z]+) /oxgc)                                      { $slash = 'm//'; return "Char::Ehp15::$1";          }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ (\w+) /oxgc)                                            { $slash = 'm//'; return "Char::Ehp15::$1(::"."$2)"; }
    elsif (/\G -(t)                            \s+ (\w+) /oxgc)                                            { $slash = 'm//'; return "-t $2";              }
    elsif (/\G \b lstat         (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Ehp15::lstat';             }
    elsif (/\G \b stat          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Ehp15::stat';              }
    ...
    elsif (/\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Ehp15::chr';               }
    elsif (/\G \b bytes::ord    (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return $function_ord;              }
    elsif (/\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Ehp15::glob';              }
    elsif (/\G \b lc \b         (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Ehp15::lc_';               }
    elsif (/\G \b lcfirst \b    (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Ehp15::lcfirst_';          }
    elsif (/\G \b uc \b         (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Ehp15::uc_';               }
    elsif (/\G \b ucfirst \b    (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Ehp15::ucfirst_';          }
    elsif (/\G \b fc \b         (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Ehp15::fc_';               }
    elsif (/\G    (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+)
                           \b    (?! \s* => )                        /oxgc) { $slash = 'm//'; return "Char::Ehp15::filetest_(qw($1))"; }
    elsif (/\G    -([rwxoRWXOezsfdlpSbcugkTBMAC])
                           \b    (?! \s* => )                        /oxgc) { $slash = 'm//'; return "Char::Ehp15::${1}_";             }
    elsif (/\G \b lstat \b      (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Ehp15::lstat_';            }
    elsif (/\G \b stat \b       (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Ehp15::stat_';             }
    ...
    elsif (/\G \b chr \b           (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Ehp15::chr_';              }
    elsif (/\G \b bytes::ord \b    (?! \s* => )                      /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord \b           (?! \s* => )                      /oxgc) { $slash = 'div'; return $function_ord_;             }
    elsif (/\G \b glob \b          (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Ehp15::glob_';             }
    elsif (/\G \b reverse \b       (?! \s* => )                      /oxgc) { $slash = 'm//'; return $function_reverse;          }
    elsif (/\G \b getc \b          (?! \s* => )                      /oxgc) { $slash = 'm//'; return $function_getc;             }
    elsif (/\G \b opendir (\s* \( \s*) (?=[A-Za-z_])                 /oxgc) { $slash = 'm//'; return "Char::Ehp15::opendir$1*";        }
    elsif (/\G \b opendir (\s+)        (?=[A-Za-z_])                 /oxgc) { $slash = 'm//'; return "Char::Ehp15::opendir$1*";        }
    elsif (/\G \b unlink \b     (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Ehp15::unlink';            }

# chdir
    elsif (/\G \b (chdir) \b    (?! \s* => ) /oxgc) {
        $slash = 'm//';

        my $e = 'Char::Ehp15::chdir';
...
# end of split
        if    (/\G (?= [,;\)\}\]] )          /oxgc) { return 'Char::Ehp15::split' . $e;                 }

# split scalar value
        elsif (/\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { return 'Char::Ehp15::split' . $e . e_string($1);  }

# split literal space
        elsif (/\G \b qq       (\#) [ ] (\#) /oxgc) { return 'Char::Ehp15::split' . $e . qq  {qq$1 $2}; }
        elsif (/\G \b qq (\s*) (\() [ ] (\)) /oxgc) { return 'Char::Ehp15::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\{) [ ] (\}) /oxgc) { return 'Char::Ehp15::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\[) [ ] (\]) /oxgc) { return 'Char::Ehp15::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\<) [ ] (\>) /oxgc) { return 'Char::Ehp15::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\S) [ ] (\2) /oxgc) { return 'Char::Ehp15::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b q        (\#) [ ] (\#) /oxgc) { return 'Char::Ehp15::split' . $e . qq   {q$1 $2}; }
        elsif (/\G \b q  (\s*) (\() [ ] (\)) /oxgc) { return 'Char::Ehp15::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\{) [ ] (\}) /oxgc) { return 'Char::Ehp15::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\[) [ ] (\]) /oxgc) { return 'Char::Ehp15::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\<) [ ] (\>) /oxgc) { return 'Char::Ehp15::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\S) [ ] (\2) /oxgc) { return 'Char::Ehp15::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G                ' [ ] '    /oxgc) { return 'Char::Ehp15::split' . $e . qq     {' '};  }
        elsif (/\G                " [ ] "    /oxgc) { return 'Char::Ehp15::split' . $e . qq     {" "};  }
...
# do
    ...
    elsif (/\G \b do (?= \s+ (?: q|qq|qx) \b)        /oxmsgc)                                  { return 'Char::Ehp15::do';         }
    elsif (/\G \b do (?= \s+ \w+)                    /oxmsgc)                                  { return 'do';                }
    elsif (/\G \b do (?= \s* \$ \w+ (?: ::\w+)* \( ) /oxmsgc)                                  { return 'do';                }
    elsif (/\G \b do \b                              /oxmsgc)                                  { return 'Char::Ehp15::do';         }
...
# require else
    elsif (/\G \b require                                   \s* ; /oxmsgc)                     { return 'Char::Ehp15::require;';   }
    elsif (/\G \b require \b                                      /oxmsgc)                     { return 'Char::Ehp15::require';    }
...
# <WILDCARD> --- glob
    ...
    elsif (/\G < ((?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[^\x80-\xA0\xE0-\xFE>\0\a\e\f\n\r\t])+?) > /oxgc) {
        return 'Char::Ehp15::glob("' . $1 . '")';
    }
...
# functions of package Char::Ehp15
        ...
        elsif ($string =~ /\G \b chop \b                                      /oxgc) { $e_string .= 'Char::Ehp15::chop';    $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::index \b                              /oxgc) { $e_string .= 'index';          $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::HP15::index \b                               /oxgc) { $e_string .= 'Char::HP15::index';    $slash = 'm//'; }
        elsif ($string =~ /\G \b index \b                                     /oxgc) { $e_string .= 'Char::Ehp15::index';   $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::rindex \b                             /oxgc) { $e_string .= 'rindex';         $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::HP15::rindex \b                              /oxgc) { $e_string .= 'Char::HP15::rindex';   $slash = 'm//'; }
        elsif ($string =~ /\G \b rindex \b                                    /oxgc) { $e_string .= 'Char::Ehp15::rindex';  $slash = 'm//'; }
        elsif ($string =~ /\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Ehp15::lc';      $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Ehp15::lcfirst'; $slash = 'm//'; }
        elsif ($string =~ /\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Ehp15::uc';      $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Ehp15::ucfirst'; $slash = 'm//'; }
        elsif ($string =~ /\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Ehp15::fc';      $slash = 'm//'; }
        elsif ($string =~ /\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+)
                                                                        \s* (\") ((?:$qq_char)+?)             (\") /oxgc) { $e_string .= "Char::Ehp15::filetest(qw($1)," . e_qq('',  $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\#) ((?:$qq_char)+?)             (\#) /oxgc) { $e_string .= "Char::Ehp15::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\() ((?:$qq_paren)+?)            (\)) /oxgc) { $e_string .= "Char::Ehp15::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\{) ((?:$qq_brace)+?)            (\}) /oxgc) { $e_string .= "Char::Ehp15::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\[) ((?:$qq_bracket)+?)          (\]) /oxgc) { $e_string .= "Char::Ehp15::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\<) ((?:$qq_angle)+?)            (\>) /oxgc) { $e_string .= "Char::Ehp15::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc) { $e_string .= "Char::Ehp15::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }

        elsif ($string =~ /\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+)
                                                                        \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc) { $e_string .= "Char::Ehp15::filetest(qw($1)," . e_q ('',  $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\#) ((?:\\\#|\\\\|$q_char)+?)    (\#) /oxgc) { $e_string .= "Char::Ehp15::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\() ((?:\\\)|\\\\|$q_paren)+?)   (\)) /oxgc) { $e_string .= "Char::Ehp15::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\{) ((?:\\\}|\\\\|$q_brace)+?)   (\}) /oxgc) { $e_string .= "Char::Ehp15::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\[) ((?:\\\]|\\\\|$q_bracket)+?) (\]) /oxgc) { $e_string .= "Char::Ehp15::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\<) ((?:\\\>|\\\\|$q_angle)+?)   (\>) /oxgc) { $e_string .= "Char::Ehp15::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc) { $e_string .= "Char::Ehp15::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }

        elsif ($string =~ /\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+) (\$ \w+(?: ::\w+)* (?: (?: ->)? (?: \( (?:$qq_paren)*? \) | \{ (?:$qq_brace)+? \} | \[ (?:$qq_bracket)+? \] ) )*) /oxgc)
                                                                                                                          { $e_string .= "Char::Ehp15::filetest(qw($1),$2)"; $slash = 'm//'; }
        elsif ($string =~ /\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+) \( ((?:$qq_paren)*?) \) /oxgc)
                                                                                                                          { $e_string .= "Char::Ehp15::filetest(qw($1),$2)"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) (?= [a-z]+) /oxgc)                                   { $e_string .= "Char::Ehp15::filetest qw($1),";    $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) (\w+) /oxgc)                                         { $e_string .= "Char::Ehp15::filetest(qw($1),$2)"; $slash = 'm//'; }

        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\") ((?:$qq_char)+?)             (\") /oxgc)    { $e_string .= "Char::Ehp15::$1(" . e_qq('',  $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\#) ((?:$qq_char)+?)             (\#) /oxgc)    { $e_string .= "Char::Ehp15::$1(" . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\() ((?:$qq_paren)+?)            (\)) /oxgc)    { $e_string .= "Char::Ehp15::$1(" . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\{) ((?:$qq_brace)+?)            (\}) /oxgc)    { $e_string .= "Char::Ehp15::$1(" . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\[) ((?:$qq_bracket)+?)          (\]) /oxgc)    { $e_string .= "Char::Ehp15::$1(" . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\<) ((?:$qq_angle)+?)            (\>) /oxgc)    { $e_string .= "Char::Ehp15::$1(" . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc)    { $e_string .= "Char::Ehp15::$1(" . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }

        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc)    { $e_string .= "Char::Ehp15::$1(" . e_q ('',  $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\#) ((?:\\\#|\\\\|$q_char)+?)    (\#) /oxgc)    { $e_string .= "Char::Ehp15::$1(" . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\() ((?:\\\)|\\\\|$q_paren)+?)   (\)) /oxgc)    { $e_string .= "Char::Ehp15::$1(" . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\{) ((?:\\\}|\\\\|$q_brace)+?)   (\}) /oxgc)    { $e_string .= "Char::Ehp15::$1(" . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\[) ((?:\\\]|\\\\|$q_bracket)+?) (\]) /oxgc)    { $e_string .= "Char::Ehp15::$1(" . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\<) ((?:\\\>|\\\\|$q_angle)+?)   (\>) /oxgc)    { $e_string .= "Char::Ehp15::$1(" . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc)    { $e_string .= "Char::Ehp15::$1(" . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }

        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s* (\$ \w+(?: ::\w+)* (?: (?: ->)? (?: \( (?:$qq_paren)*? \) | \{ (?:$qq_brace)+? \} | \[ (?:$qq_bracket)+? \] ) )*) /oxgc)
                                                                                                                          { $e_string .= "Char::Ehp15::$1($2)";      $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s* \( ((?:$qq_paren)*?) \) /oxgc)                          { $e_string .= "Char::Ehp15::$1($2)";      $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) (?= \s+ [a-z]+) /oxgc)                                      { $e_string .= "Char::Ehp15::$1";          $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ (\w+) /oxgc)                                            { $e_string .= "Char::Ehp15::$1(::"."$2)"; $slash = 'm//'; }
        elsif ($string =~ /\G -(t)                            \s+ (\w+) /oxgc)                                            { $e_string .= "-t $2";              $slash = 'm//'; }
        elsif ($string =~ /\G \b lstat         (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Ehp15::lstat';             $slash = 'm//'; }
        elsif ($string =~ /\G \b stat          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Ehp15::stat';              $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Ehp15::chr';               $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Ehp15::glob';              $slash = 'm//'; }
        elsif ($string =~ /\G \b lc \b                                              /oxgc) { $e_string .= 'Char::Ehp15::lc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst \b                                         /oxgc) { $e_string .= 'Char::Ehp15::lcfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b uc \b                                              /oxgc) { $e_string .= 'Char::Ehp15::uc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst \b                                         /oxgc) { $e_string .= 'Char::Ehp15::ucfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b fc \b                                              /oxgc) { $e_string .= 'Char::Ehp15::fc_';               $slash = 'm//'; }
        elsif ($string =~ /\G    (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+)
                                                                 \b                 /oxgc) { $e_string .= "Char::Ehp15::filetest_(qw($1))"; $slash = 'm//'; }
        elsif ($string =~ /\G    -([rwxoRWXOezsfdlpSbcugkTBMAC]) \b                 /oxgc) { $e_string .= "Char::Ehp15::${1}_";             $slash = 'm//'; }
        elsif ($string =~ /\G \b lstat \b                                           /oxgc) { $e_string .= 'Char::Ehp15::lstat_';            $slash = 'm//'; }
        elsif ($string =~ /\G \b stat \b                                            /oxgc) { $e_string .= 'Char::Ehp15::stat_';             $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr \b                                             /oxgc) { $e_string .= 'Char::Ehp15::chr_';              $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::ord \b                                      /oxgc) { $e_string .= 'ord';                      $slash = 'div'; }
        elsif ($string =~ /\G \b ord \b                                             /oxgc) { $e_string .= $function_ord_;             $slash = 'div'; }
        elsif ($string =~ /\G \b glob \b                                            /oxgc) { $e_string .= 'Char::Ehp15::glob_';             $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b opendir (\s* \( \s*) (?=[A-Za-z_])                 /oxgc) { $e_string .= "Char::Ehp15::opendir$1*";        $slash = 'm//'; }
        elsif ($string =~ /\G \b opendir (\s+)        (?=[A-Za-z_])                 /oxgc) { $e_string .= "Char::Ehp15::opendir$1*";        $slash = 'm//'; }
        elsif ($string =~ /\G \b unlink \b                                          /oxgc) { $e_string .= 'Char::Ehp15::unlink';            $slash = 'm//'; }

# chdir
        elsif ($string =~ /\G \b (chdir) \b (?! \s* => ) /oxgc) {
            $slash = 'm//';

            $e_string .= 'Char::Ehp15::chdir';
...
# end of split
            if    ($string =~ /\G (?= [,;\)\}\]] )          /oxgc) { return 'Char::Ehp15::split' . $e;                                           }

# split scalar value
            elsif ($string =~ /\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { $e_string .= 'Char::Ehp15::split' . $e . e_string($1);  next E_STRING_LOOP; }

# split literal space
            elsif ($string =~ /\G \b qq       (\#) [ ] (\#) /oxgc) { $e_string .= 'Char::Ehp15::split' . $e . qq  {qq$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\() [ ] (\)) /oxgc) { $e_string .= 'Char::Ehp15::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\{) [ ] (\}) /oxgc) { $e_string .= 'Char::Ehp15::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\[) [ ] (\]) /oxgc) { $e_string .= 'Char::Ehp15::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\<) [ ] (\>) /oxgc) { $e_string .= 'Char::Ehp15::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\S) [ ] (\2) /oxgc) { $e_string .= 'Char::Ehp15::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q        (\#) [ ] (\#) /oxgc) { $e_string .= 'Char::Ehp15::split' . $e . qq   {q$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\() [ ] (\)) /oxgc) { $e_string .= 'Char::Ehp15::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\{) [ ] (\}) /oxgc) { $e_string .= 'Char::Ehp15::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\[) [ ] (\]) /oxgc) { $e_string .= 'Char::Ehp15::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\<) [ ] (\>) /oxgc) { $e_string .= 'Char::Ehp15::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\S) [ ] (\2) /oxgc) { $e_string .= 'Char::Ehp15::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G                ' [ ] '    /oxgc) { $e_string .= 'Char::Ehp15::split' . $e . qq     {' '};  next E_STRING_LOOP; }
            elsif ($string =~ /\G                " [ ] "    /oxgc) { $e_string .= 'Char::Ehp15::split' . $e . qq     {" "};  next E_STRING_LOOP; }
...
sub character_class {
    ...
    if ($char eq '.') {
        if ($modifier =~ /s/) {
            return '${Char::Ehp15::dot_s}';
        }
        else {
            return '${Char::Ehp15::dot}';
        }
    }
    else {
        return Char::Ehp15::classic_character_class($char);
    }
}
...
sub e_capture {
    ...
    return join '', '${Char::Ehp15::capture(', $_[0], ')}';
    return join '', '$',                 $_[0];
}
...
sub e_tr {
    ...
    if ($modifier =~ tr/bB//d) {
        ...
        if ($variable eq '') {
            $e_tr = qq{Char::Ehp15::tr(\$_,' =~ ',$charclass,$e$charclass2,'$modifier')};
        }
        else {
            $e_tr = qq{Char::Ehp15::tr($variable,'$bind_operator',$charclass,$e$charclass2,'$modifier')};
        }
    }
...
sub e_qq {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # octal escape sequence
            ...
            $char[$i] = Char::Ehp15::octchr($1);
        }

        # hexadecimal escape sequence
        elsif ($char[$i] =~ /\A \\x \{ ([0-9A-Fa-f]+) \} \z/oxms) {
            $char[$i] = Char::Ehp15::hexchr($1);
        }
        ...
        # \u \l \U \L \F \Q \E
            ...
            $char[$i] = '@{[Char::Ehp15::ucfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\l') {
            $char[$i] = '@{[Char::Ehp15::lcfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\U') {
            $char[$i] = '@{[Char::Ehp15::uc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\L') {
            $char[$i] = '@{[Char::Ehp15::lc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\F') {
            $char[$i] = '@{[Char::Ehp15::fc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\Q') {
...
sub e_qr {
    ...
    my @char = $string =~ /\G(
        ...
        \\c   [\x40-\x5F]          |
        \\N\{ [^\x80-\xA0\xE0-\xFE0-9\}][^\x80-\xA0\xE0-\xFE\}]* \} |
        \\p\{ [^\x80-\xA0\xE0-\xFE0-9\}][^\x80-\xA0\xE0-\xFE\}]* \} |
        \\P\{ [^\x80-\xA0\xE0-\xFE0-9\}][^\x80-\xA0\xE0-\xFE\}]* \} |
        \\  (?:$q_char)            |
        \$` | \$\{`\} | \$ \s* PREMATCH  | \$ \s* \{ \s* PREMATCH  \s* \} | \$ \s* \{\^PREMATCH\}  |
        \$& | \$\{&\} | \$ \s* MATCH     | \$ \s* \{ \s* MATCH     \s* \} | \$ \s* \{\^MATCH\}     |
    ...
    for (my $i=0; $i <= $#char; $i++) {
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Char::Ehp15::charlist_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Char::Ehp15::charlist_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # open character class [^...]
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    # [^...]
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Char::Ehp15::charlist_not_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Char::Ehp15::charlist_not_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ /\A [\x00-\xFF] \z/oxms) and (Char::Ehp15::uc($char[$i]) ne Char::Ehp15::fc($char[$i]))) {
            if (CORE::length(Char::Ehp15::fc($char[$i])) == 1) {
                $char[$i] = '['   . Char::Ehp15::uc($char[$i])       . Char::Ehp15::fc($char[$i]) . ']';
            }
            else {
                $char[$i] = '(?:' . Char::Ehp15::uc($char[$i]) . '|' . Char::Ehp15::fc($char[$i]) . ')';
            }
        ...
        # $0 --> $0
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Ehp15::ignorecase(' . $char[$i] . ')]}';
            }
        }
        elsif ($char[$i] =~ /\A \$ \{ \s* 0 \s* \} \z/oxms) {
            if ($ignorecase) {
        ...
        # $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Char::Ehp15::PREMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Ehp15::ignorecase(Char::Ehp15::PREMATCH())]}';
            }
            else {
                $char[$i] = '@{[Char::Ehp15::PREMATCH()]}';
            }
        ...
        # $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Char::Ehp15::MATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Ehp15::ignorecase(Char::Ehp15::MATCH())]}';
            }
            else {
                $char[$i] = '@{[Char::Ehp15::MATCH()]}';
            }
        ...
        # $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Char::Ehp15::POSTMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Ehp15::ignorecase(Char::Ehp15::POSTMATCH())]}';
            }
            else {
                $char[$i] = '@{[Char::Ehp15::POSTMATCH()]}';
            }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr with multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                                              2   3                                 4   5
                q<eval{local $Char::HP15::re_t=%s; local $Char::HP15::re_a=''; while($Char::HP15::re_t =~ %s){%s local $^W=0; local $Char::HP15::re_r=%s; %s$Char::HP15::re_t="$Char::HP15::re_a${1}$Char::HP15::re_r$'"; pos($Char::HP15::re_t)=length "$Char::HP15::re_a${1}$Char::HP15::re_r"; $Char::HP15::re_a=substr($Char::HP15::re_t,0,pos($Char::HP15::re_t)); } return $Char::HP15::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Char::HP15::re_r=eval $Char::HP15::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Char::HP15::re_t=%s; while($Char::HP15::re_t =~ %s){%s local $^W=0; local $Char::HP15::re_r=%s; %s$Char::HP15::re_t="$`$Char::HP15::re_r$'"; pos($Char::HP15::re_t)=length "$`$Char::HP15::re_r"; } return $Char::HP15::re_t}>,
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Char::HP15::re_r=%s; %s"%s$Char::HP15::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g with multibyte anchoring
            ...
            $sub = sprintf(
                ...
                q<eval{local $Char::HP15::re_n=0; local $Char::HP15::re_a=''; while(%s =~ %s){%s local $^W=0; local $Char::HP15::re_r=%s; %s%s="$Char::HP15::re_a${1}$Char::HP15::re_r$'"; pos(%s)=length "$Char::HP15::re_a${1}$Char::HP15::re_r"; $Char::HP15::re_a=substr(%s,0,pos(%s)); $Char::HP15::re_n++} return %s$Char::HP15::re_n}>,
...
# (and so on)
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Char::HP15::re_n=0; while(%s =~ %s){%s local $^W=0; local $Char::HP15::re_r=%s; %s%s="$`$Char::HP15::re_r$'"; pos(%s)=length "$`$Char::HP15::re_r"; $Char::HP15::re_n++} return %s$Char::HP15::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Char::HP15::re_r=%s; %s%s="%s$Char::HP15::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Char::HP15::re_r=%s; %s%s="%s$Char::HP15::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Char::HP15::re_r=eval $Char::HP15::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
sub e_chdir_q {
    ...
    if ($^W) {
        if (Char::Ehp15::_MSWin32_5Cended_path($string)) {
            if ($] !~ /^5\.005/) {
                warn <<END;
...
sub e_split {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # ${ foo }
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Ehp15::ignorecase(' . $1 . ')]}';
            }
        }
    ...
    $modifier =~ tr/i//d;
    if ($left_e > $right_e) {
        return join '', 'Char::Ehp15::split', $ope, $delimiter, @char, '>]}' x ($left_e - $right_e), $end_delimiter, $modifier;
    }
    return     join '', 'Char::Ehp15::split', $ope, $delimiter, @char,                               $end_delimiter, $modifier;
}
    ...
    for my $realfilename (_realfilename($expr)) {
        ...
        if (Char::Ehp15::_open_r($fh, $realfilename)) {
            ...
            if ($script =~ /^ \s* use \s+ Char::HP15 \s* ([^\x80-\xA0\xE0-\xFE;]*) ; \s* \n? $/oxms) {
                ...
                return qq[BEGIN { Char::Ehp15::require '$expr'; $module->import() if $module->can('import'); }];
            }
            last;
        }
    }
...
sub e_no_noparam {
    ...
    for my $realfilename (_realfilename($expr)) {
        ...
        if (Char::Ehp15::_open_r($fh, $realfilename)) {
            ...
            if ($script =~ /^ \s* use \s+ Char::HP15 \s* ([^\x80-\xA0\xE0-\xFE;]*) ; \s* \n? $/oxms) {
                return qq[BEGIN { Char::Ehp15::require '$expr'; $module->unimport() if $module->can('unimport'); }];
            }
            last;
        }
    }
...
sub e_use {
    ...
    for my $realfilename (_realfilename($expr)) {
        ...
        if (Char::Ehp15::_open_r($fh, $realfilename)) {
            ...
            if ($script =~ /^ \s* use \s+ Char::HP15 \s* ([^\x80-\xA0\xE0-\xFE;]*) ; \s* \n? $/oxms) {
                return qq[BEGIN { Char::Ehp15::require '$expr'; $module->import($list) if $module->can('import'); }];
            }
            last;
        }
    }
...
sub e_no {
    ...
    for my $realfilename (_realfilename($expr)) {
        ...
        if (Char::Ehp15::_open_r($fh, $realfilename)) {
            ...
            if ($script =~ /^ \s* use \s+ Char::HP15 \s* ([^\x80-\xA0\xE0-\xFE;]*) ; \s* \n? $/oxms) {
                return qq[BEGIN { Char::Ehp15::require '$expr'; $module->unimport($list) if $module->can('unimport'); }];
            }
            last;
        }
    }
...
=item * You'll have to download Char/HP15.pm and Char/Ehp15.pm and put it in your perl lib directory.

=item * You'll need to write "use Char::HP15;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Char::HP15;
  use Char::HP15 ver.sion;             --- require minimum version
  use Char::HP15 ver.sion.0;           --- expects version (match or die)
  use Char::HP15 qw(ord reverse getc); --- demand enhanced feature of ord, reverse, and getc
  use Char::HP15 ver.sion qw(ord reverse getc);
  use Char::HP15 ver.sion.0 qw(ord reverse getc);

  # "no Char::HP15;" not supported

  or

  $ perl Char/HP15.pm HP-15_script.pl > Escaped_script.pl.e
...
Char::HP15 software is "middleware" between perl interpreter and your Perl script
written in HP-15.
...
By "use Char::HP15;", it automatically interpret your script as HP-15. The various
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Char/HP15.pm, Char/Ehp15.pm
  +---------------------------------------+
...
expand wildcard arguments supplied to programs. Instead, import of Char/Ehp15.pm
works well.
...
=head1 Software Composition
   ...
   Char/HP15.pm               --- source code filter to escape HP-15
   Char/Ehp15.pm              --- run-time routines for Char/HP15.pm
...
You need write 'use Char::HP15;' in your script.

  ---------------------------------
  Before      You do
  ---------------------------------
  (nothing)   use Char::HP15;
  ---------------------------------

=head1 Escaping Multiple-Octet Code (Char/HP15.pm provides)
...
=head1 Multiple-Octet Anchoring of Regular Expression (Char/HP15.pm provides)

Char/HP15.pm applies multiple-octet anchoring at beginning of regular expression.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/regexp/               m/${Char::Ehp15::anchor}(?:regexp).../
  --------------------------------------------------------------------------------

=head1 Escaping Second Octet (Char/HP15.pm provides)

Char/HP15.pm escapes second octet of multiple-octet character in regular expression.
...
=head1 Multiple-Octet Character Regular Expression (Char/HP15.pm provides)

Char/HP15.pm clusters multiple-octet character with quantifier, makes cluster from
...
from classic Perl character class shortcuts and POSIX-style character classes.
  ...
  m/...\D.../             m/...(?:${Char::Ehp15::eD}).../
  m/...[[:^digit:]].../   m/...(?:${Char::Ehp15::not_digit}).../
  --------------------------------------------------------------------------------

=head1 Calling 'Char::Ehp15::ignorecase()' (Char/HP15.pm provides)

Char/HP15.pm applies calling 'Char::Ehp15::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Char::Ehp15::ignorecase($var)]}.../
  --------------------------------------------------------------------------------
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  Before                  After
  --------------------------------------------------------------------------------
  /regexp/                /ditto$Char::Ehp15::matched/
  m/regexp/               m/ditto$Char::Ehp15::matched/
  ?regexp?                m?ditto$Char::Ehp15::matched?
  m?regexp?               m?ditto$Char::Ehp15::matched?
 
  $_ =~                   ($_ =~ m/ditto$Char::Ehp15::matched/) ?
  s/regexp/replacement/   eval{ Char::Ehp15::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
 
  $_ !~                   ($_ !~ m/ditto$Char::Ehp15::matched/) ?
  s/regexp/replacement/   1 :
                          eval{ Char::Ehp15::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; undef }
 
  split(/regexp/)         Char::Ehp15::split(qr/regexp/)
  split(m/regexp/)        Char::Ehp15::split(qr/regexp/)
  split(qr/regexp/)       Char::Ehp15::split(qr/regexp/)
  qr/regexp/              qr/ditto$Char::Ehp15::matched/
  --------------------------------------------------------------------------------
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  Before                  After
  --------------------------------------------------------------------------------
  /regexp/b               /(?:regexp)$Char::Ehp15::matched/
  m/regexp/b              m/(?:regexp)$Char::Ehp15::matched/
  ?regexp?b               m?regexp$Char::Ehp15::matched?
  m?regexp?b              m?regexp$Char::Ehp15::matched?
 
  $_ =~                   ($_ =~ m/(\G[\x00-\xFF]*?)(?:regexp)$Char::Ehp15::matched/) ?
  s/regexp/replacement/b  eval{ Char::Ehp15::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
  ...
  qr/regexp/b             qr/(?:regexp)$Char::Ehp15::matched/
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Char/Ehp15.pm provides)
...
Definitions in Char/Ehp15.pm.
  ...
  ${Char::Ehp15::eS}             qr{(?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[^\x80-\xA0\xE0-\xFE\s])}
  ...
  ${Char::Ehp15::not_space}      qr{(?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[^\x80-\xA0\xE0-\xFE\s\x0B])}
...
=head1 Un-Escaping \ of \N, \p, \P and \X (Char/HP15.pm provides)

Char/HP15.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char/HP15.pm and Char/Ehp15.pm provide)

Insert 'Char::Ehp15::' at head of function name. Char/Ehp15.pm provides your script Char::Ehp15::*
functions.
  ...
  substr      substr           Byte
  pos         pos              Byte
  split       Char::Ehp15::split     Character
  tr///       Char::Ehp15::tr        Character
  tr///b      tr///            Byte
  tr///B      tr///            Byte
  y///        Char::Ehp15::tr        Character
  y///b       tr///            Byte
  y///B       tr///            Byte
  chop        Char::Ehp15::chop      Character
  index       Char::Ehp15::index     Character
  rindex      Char::Ehp15::rindex    Character
  lc          Char::Ehp15::lc        Character
  lcfirst     Char::Ehp15::lcfirst   Character
  uc          Char::Ehp15::uc        Character
  ucfirst     Char::Ehp15::ucfirst   Character
  fc          Char::Ehp15::fc        Character
  chr         Char::Ehp15::chr       Character
  glob        Char::Ehp15::glob      Character
  lstat       Char::Ehp15::lstat     Character
  opendir     Char::Ehp15::opendir   Character
  stat        Char::Ehp15::stat      Character
  unlink      Char::Ehp15::unlink    Character
  chdir       Char::Ehp15::chdir     Character
  do          Char::Ehp15::do        Character
  require     Char::Ehp15::require   Character
  -------------------------------------------

  ------------------------------------------------------------------------------------------------------------------------
  Before                   After
  ------------------------------------------------------------------------------------------------------------------------
  use Perl::Module;        BEGIN { Char::Ehp15::require 'Perl/Module.pm'; Perl::Module->import() if Perl::Module->can('import'); }
  use Perl::Module @list;  BEGIN { Char::Ehp15::require 'Perl/Module.pm'; Perl::Module->import(@list) if Perl::Module->can('import'); }
  use Perl::Module ();     BEGIN { Char::Ehp15::require 'Perl/Module.pm'; }
  no Perl::Module;         BEGIN { Char::Ehp15::require 'Perl/Module.pm'; Perl::Module->unimport() if Perl::Module->can('unimport'); }
  no Perl::Module @list;   BEGIN { Char::Ehp15::require 'Perl/Module.pm'; Perl::Module->unimport(@list) if Perl::Module->can('unimport'); }
  no Perl::Module ();      BEGIN { Char::Ehp15::require 'Perl/Module.pm'; }
  ------------------------------------------------------------------------------------------------------------------------

=head1 Escaping File Test Operators (Char/HP15.pm and Char/Ehp15.pm provide)

Insert 'Char::Ehp15::' instead of '-' of operator.

  Available in MSWin32, MacOS, and UNIX-like systems
  --------------------------------------------------------------------------
  Before   After      Meaning
  --------------------------------------------------------------------------
  -r       Char::Ehp15::r   File or directory is readable by this (effective) user or group
  -w       Char::Ehp15::w   File or directory is writable by this (effective) user or group
  -e       Char::Ehp15::e   File or directory name exists
  -x       Char::Ehp15::x   File or directory is executable by this (effective) user or group
  -z       Char::Ehp15::z   File exists and has zero size (always false for directories)
  -f       Char::Ehp15::f   Entry is a plain file
  -d       Char::Ehp15::d   Entry is a directory
  -t       -t         The filehandle is a TTY (as reported by the isatty() system function;
                      filenames can't be tested by this test)
  -T       Char::Ehp15::T   File looks like a "text" file
  -B       Char::Ehp15::B   File looks like a "binary" file
  -M       Char::Ehp15::M   Modification age (measured in days)
  -A       Char::Ehp15::A   Access age (measured in days)
  -C       Char::Ehp15::C   Inode-modification age (measured in days)
  -s       Char::Ehp15::s   File or directory exists and has nonzero size
  ...
  Before   After      Meaning
  --------------------------------------------------------------------------
  -R       Char::Ehp15::R   File or directory is readable by this real user or group
  -W       Char::Ehp15::W   File or directory is writable by this real user or group
  -X       Char::Ehp15::X   File or directory is executable by this real user or group
  -l       Char::Ehp15::l   Entry is a symbolic link
  -S       Char::Ehp15::S   Entry is a socket
  --------------------------------------------------------------------------
  ...
  Before   After      Meaning
  --------------------------------------------------------------------------
  -o       Char::Ehp15::o   File or directory is owned by this (effective) user
  -O       Char::Ehp15::O   File or directory is owned by this real user
  -p       Char::Ehp15::p   Entry is a named pipe (a "fifo")
  -b       Char::Ehp15::b   Entry is a block-special file (like a mountable disk)
  -c       Char::Ehp15::c   Entry is a character-special file (like an I/O device)
  -u       Char::Ehp15::u   File or directory is setuid
  -g       Char::Ehp15::g   File or directory is setgid
  -k       Char::Ehp15::k   File or directory has the sticky bit set
  --------------------------------------------------------------------------
...
You need write 'Char::HP15::' at head of function name when you want character-
oriented function. See 'Character-Oriented Functions'.
  ...
  --------------------------------------------------------
  ord        Char::HP15::ord
  reverse    Char::HP15::reverse
  getc       Char::HP15::getc
  length     Char::HP15::length
  substr     Char::HP15::substr
  index      Char::HP15::index          See 'About Indexes'
  rindex     Char::HP15::rindex         See 'About Rindexes'
  --------------------------------------------------------
  ...
  (same as Char::Ehp15::index)
  Char::HP15::index    Character   Character    Character-oriented semantics
  ...
  (same as Char::Ehp15::rindex)
  Char::HP15::rindex   Character   Character    Character-oriented semantics
...
=item * Ordinal Value of Character
  ...
  If you import ord "use Char::HP15 qw(ord);", ord of your script will be rewritten in
  Char::HP15::ord. Char::HP15::ord is not compatible with ord of JPerl.

=item * Reverse List or String

  @reverse = Char::HP15::reverse(@list);
  $reverse = Char::HP15::reverse(@list);
  ...
  If you import reverse "use Char::HP15 qw(reverse);", reverse of your script will be
  rewritten in Char::HP15::reverse. Char::HP15::reverse is not compatible with reverse of
  JPerl.
...
=item * Returns Next Character
  ...
  $getc = Char::HP15::getc(FILEHANDLE);
  $getc = Char::HP15::getc($filehandle);
  $getc = Char::HP15::getc;
  ...
  If you import getc "use Char::HP15 qw(getc);", getc of your script will be rewritten in
  Char::HP15::getc. Char::HP15::getc is not compatible with getc of JPerl.

=item * Length by HP-15 Character

  $length = Char::HP15::length($string);
  $length = Char::HP15::length();

  This function returns the length in characters (programmer-visible characters) of
  the scalar value $string. If $string is omitted, it returns the Char::HP15::length of
  $_.

  Do not try to use Char::HP15::length to find the size of an array or hash. Use scalar
...
=item * Substr by HP-15 Character
  ...
  $substr = Char::HP15::substr($string,$offset,$length,$replacement);
  $substr = Char::HP15::substr($string,$offset,$length);
  $substr = Char::HP15::substr($string,$offset);
  ...
  my $color  = Char::HP15::substr $s, 4, 5;      # black
  my $middle = Char::HP15::substr $s, 4, -11;    # black cat climbed the
  my $end    = Char::HP15::substr $s, 14;        # climbed the green tree
  my $tail   = Char::HP15::substr $s, -4;        # tree
  my $z      = Char::HP15::substr $s, -4, 2;     # tr

  If Perl version 5.14 or later, you can use the Char::HP15::substr() function as an
  ...
  end of the string, Char::HP15::substr() returns the undefined value and produces a
  ...
  my $name = 'fred';
  Char::HP15::substr($name, 4) = 'dy';         # $name is now 'freddy'
  my $null = Char::HP15::substr $name, 6, 2;   # returns "" (no warning)
  my $oops = Char::HP15::substr $name, 7;      # returns undef, with warning
  Char::HP15::substr($name, 7) = 'gap';        # raises an exception

  An alternative to using Char::HP15::substr() as an lvalue is to specify the replacement
  ...
  my $s = "The black cat climbed the green tree";
  my $z = Char::HP15::substr $s, 14, 7, "jumped from";    # climbed
  # $s is now "The black cat jumped from the green tree"

  Note that the lvalue returned by the three-argument version of Char::HP15::substr() acts
...
=item * Index by HP-15 Character
  ...
  $index = Char::HP15::index($string,$substring,$offset);
  $index = Char::HP15::index($string,$substring);
  ...
  $pos = -1;
  while (($pos = Char::HP15::index($string, $lookfor, $pos)) > -1) {
      print "Found at $pos\n";
      $pos++;
  }
...
=item * Rindex by HP-15 Character
  ...
  $rindex = Char::HP15::rindex($string,$substring,$offset);
  $rindex = Char::HP15::rindex($string,$substring);

  This function works just like Char::HP15::index except that it returns the character
  position of the last occurrence of $substring in $string (a reverse Char::HP15::index).
  ...
  $pos = Char::HP15::length($string);
  while (($pos = Char::HP15::rindex($string, $lookfor, $pos)) >= 0) {
      print "Found at $pos\n";
      $pos--;
  }
...
=item * Filename Globbing
  ...
  This operator is implemented via the Char::Ehp15::glob() function. See Char::Ehp15::glob of
  Char/Ehp15.pm for details.
...
=item * Ordinal Value of Byte
  ...
  of "use Char::HP15 qw(ord);" exists or not. If $expr is omitted, it uses $_.
  The return value is always unsigned.
...
=item * Reverse List or Byte String
  ...
  "use Char::HP15 qw(reverse);" exists or not.
...
=head1 Un-Escaping bytes::* Functions (Char/HP15.pm provides)

Char/HP15.pm removes 'bytes::' at head of function name.
...
=head1 Escaping Built-in Standard Module (Char/Ehp15.pm provides)

Char/Ehp15.pm does "BEGIN { unshift @INC, '/Perl/site/lib/Char::HP15' }" at head.
Store the standard module modified for Char::HP15 software in this directory to
override built-in standard modules.

=head1 Escaping Standard Module Content (You do)

You need copy built-in standard module to /Perl/site/lib/Char::HP15 and change
'use utf8;' to 'use Char::HP15;' in its. You need help yourself for now.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Char/Ehp15.pm provides these
  functions.
...
=item * Char::HP15::substr as Lvalue

Char::HP15::substr differs from CORE::substr, and cannot be used as a lvalue.
To change part of a string, you can use the optional fourth argument which is the
replacement string.

Char::HP15::substr($string, 13, 4, "JPerl");
...
=item * Special Variables $` and $& need /( Capture All )/
  ...
  Before          After                Works as
  -------------------------------------------------------------------------------------------
  $`              Char::Ehp15::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $PREMATCH       Char::Ehp15::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  ${^PREMATCH}    Char::Ehp15::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $&              Char::Ehp15::MATCH()       $1
  $MATCH          Char::Ehp15::MATCH()       $1
  ${^MATCH}       Char::Ehp15::MATCH()       $1
  $'              Char::Ehp15::POSTMATCH()   $'
  $POSTMATCH      Char::Ehp15::POSTMATCH()   $'
  ${^POSTMATCH}   Char::Ehp15::POSTMATCH()   $'
  -------------------------------------------------------------------------------------------
...
Even if ${^WIN32_SLOPPY_STAT} is set to a true value, file test functions Char::Ehp15::*(),
Char::Ehp15::lstat(), and Char::Ehp15::stat() on Microsoft Windows open the file for the path
...
Char::HP15 software remains one language and one interpreter.
