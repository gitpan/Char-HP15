...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Char::Ehp15::uc($char[$i]) ne Char::Ehp15::lc($char[$i]))) {
            $char[$i] = '[' . Char::Ehp15::uc($char[$i]) . Char::Ehp15::lc($char[$i]) . ']';
        }
...
sub e_require {
    ...
    return qq<Char::Ehp15::require '$expr';>;
}
...
http://search.cpan.org/dist/Char-Char::HP15/Char::HP15/JA.pm
...
Definitions in Char/Ehp15.pm.
  ...
  @{Char::Ehp15::eV}             qr{(?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[^\x80-\xA0\xE0-\xFE\x0C\x0A\x0D])}
...
=item * Order Of Character
  ...
  This function returns the numeric value ASCII or HP-15 of the first character
...
=item * Length By HP-15 Character
  ...
  $string is omitted, it returns the Char::HP15::length of $_.
...
=item * Substr By HP-15 Character
  ...
  area past the end of the string, Char::HP15::substr raises an exception.
...
=item * Rindex By HP-15 Character
  ...
  $rindex = Char::HP15::rindex($string,$substring,$position);
  $rindex = Char::HP15::rindex($string,$substring);

  This function works just like Char::HP15::index except that it returns the position of
