...
END
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                for (@list) {
                    $Char::Ehp15::function_ord     = 'Char::HP15::ord'     if /\A ord \z/oxms;
                    $Char::Ehp15::function_ord_    = 'Char::HP15::ord_'    if /\A ord \z/oxms;
                    $Char::Ehp15::function_reverse = 'Char::HP15::reverse' if /\A reverse \z/oxms;
                    $Char::Ehp15::function_getc    = 'Char::HP15::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }

    $e_script .= Char::HP15::escape();
...
=head1 Escaping Multiple-Octet Code (Char::HP15 software provides)
...
=head1 Multiple-Octet Anchoring of Regular Expression (Char::HP15 software provides)

Char::HP15 software applies multiple-octet anchoring at beginning of regular expression.
...
=head1 Escaping Second Octet (Char::HP15 software provides)

Char::HP15 software escapes second octet of multiple-octet character in regular
expression.
...
=head1 Multiple-Octet Character Regular Expression (Char::HP15 software provides)

Char::HP15 software clusters multiple-octet character with quantifier, makes cluster from
...
=head1 Calling 'Char::Ehp15::ignorecase()' (Char::HP15 software provides)

Char::HP15 software applies calling 'Char::Ehp15::ignorecase()' instead of /i modifier.
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  s/regexp/replacement/   CORE::eval{ Char::Ehp15::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
 
  $_ !~                   ($_ !~ m/ditto$Char::Ehp15::matched/) ?
  s/regexp/replacement/   1 :
                          CORE::eval{ Char::Ehp15::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; undef }
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  s/regexp/replacement/b  CORE::eval{ Char::Ehp15::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
...
=head1 Un-Escaping \ of \N, \p, \P, and \X (Char::HP15 software provides)

Char::HP15 software removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char::HP15 software provide)
...
=head1 Escaping File Test Operators (Char::HP15 software provide)
...
=item * Eval HP-15 Script
  ...
  $eval = Char::HP15::eval { block };
  $eval = Char::HP15::eval $expr;
  $eval = Char::HP15::eval;

  The Char::HP15::eval keyword serves two distinct but related purposes in JPerl.
  These purposes are represented by two forms of syntax, Char::HP15::eval { block }
  and Char::HP15::eval $expr. The first form traps runtime exceptions (errors)
  ...
  more general. Whichever form you use, Char::HP15::eval is the preferred way to do
  all exception handling in JPerl.

  For either form of Char::HP15::eval, the value returned from an Char::HP15::eval is
  ...
  Char::HP15::eval itself. See wantarray for more on how the evaluation context
  can be determined.

  If there is a trappable error (including any produced by the die operator),
  Char::HP15::eval returns undef and puts the error message (or object) in $@. If
  ...
  can test it reliably afterward for errors. A simple Boolean test suffices:
      ...
      Char::HP15::eval { ... }; # trap runtime errors
      if ($@) { ... }     # handle error

  (Prior to Perl 5.16, a bug caused undef to be returned in list context for
  syntax errors, but not for runtime errors.)

  The Char::HP15::eval { block } form is syntax checked and compiled at compile time,
  so it is just as efficient at runtime as any other block. (People familiar
  with the slow Char::HP15::eval $expr form are occasionally confused on this issue.)
  Because the { block } is compiled when the surrounding code is, this form of
  Char::HP15::eval cannot trap syntax errors.

  The Char::HP15::eval $expr form can trap syntax errors because it parses the code
  ...
  after the Char::HP15::eval is complete, as do any subroutine or format definitions.
  The code of the Char::HP15::eval is treated as a block, so any locally scoped
  variables declared within the Char::HP15::eval last only until the Char::HP15::eval is
  done. (See my and local.) As with any code in a block, a final semicolon is
  not required.

  Char::HP15::eval will be escaped as follows:

  -------------------------------------------------
  Before                  After
  -------------------------------------------------
  Char::HP15::eval { block }    eval { block }
  Char::HP15::eval $expr        eval Char::HP15::escape $expr
  Char::HP15::eval              eval Char::HP15::escape
  -------------------------------------------------

  To tell the truth, the subroutine Char::HP15::eval does not exist. If it exists,
  you will troubled, when Char::HP15::eval has a parameter that is single quoted
  string included my variables. Char::HP15::escape is a subroutine that makes Perl
  ...
  Since Char::HP15::eval traps errors that would otherwise prove fatal, it is useful
  for determining whether particular features (such as fork or symlink) are
  implemented.

  Because Char::HP15::eval { block } is syntax checked at compile time, any syntax
  error is reported earlier. Therefore, if your code is invariant and both
  Char::HP15::eval $expr and Char::HP15::eval { block } will suit your purposes equally
  well, the { block } form is preferred. For example:

      # make divide-by-zero nonfatal
      Char::HP15::eval { $answer = $a / $b; };
      warn $@ if $@;
      ...
      Char::HP15::eval { $answer = }; # WRONG

      # a runtime syntax error
      Char::HP15::eval '$answer =';   # sets $@
  ...
  Using the Char::HP15::eval { block } form as an exception trap in libraries does
  ...
  as this example shows:
      ...
      Char::HP15::eval { local $SIG{'__DIE__'}; $answer = $a / $b; };
      warn $@ if $@;
  ...
  which has the effect of changing their error messages:
      ...
      # __DIE__ hooks may modify error messages
      {
          ...
          Char::HP15::eval { die "foo lives here" };
  ...
  With an Char::HP15::eval, you should be especially careful to remember what's being
  looked at when:

      Char::HP15::eval $x;        # CASE 1
      Char::HP15::eval "$x";      # CASE 2

      Char::HP15::eval '$x';      # CASE 3
      Char::HP15::eval { $x };    # CASE 4

      Char::HP15::eval "\$$x++";  # CASE 5
      $$x++;                # CASE 6
  ...
  a temporary is required if you want to mask some but not all errors:
      ...
      # alter $@ on nefarious repugnancy only
      {
          my $e;
          {
              ...
              Char::HP15::eval { test_repugnancy() };
  ...
  The block of Char::HP15::eval { block } does not count as a loop, so the loop
...
=head1 Un-Escaping bytes::* Subroutines (Char::HP15 software provides)

Char::HP15 software removes 'bytes::' at head of subroutine name.
...
some bugs. Thus, if you happen to find a bug that's in Char::HP15 software and
