...
my $your_gap = '';
$your_gap = q{\G(?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[^\x80-\xA0\xE0-\xFE])*?};
...
my $m_matched   = '';
my $s_matched   = '';
$m_matched   = q{@Char::Ehp15::m_matched};
...
sub character_class {
    ...
    if ($char eq '.') {
        if ($modifier =~ /s/) {
            return '(?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[^\x80-\xA0\xE0-\xFE])';
        }
        else {
            return '(?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[^\x80-\xA0\xE0-\xFE\x0A])';
        }
    }
    else {
...
=item * Handles Raw HP-15, No UTF8 flag like JPerl
...
The character classes are redefined as follows to backward compatibility.
  ...
  Before        After
  ---------------------------------------------------------------------------
   .            (?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[^\x80-\xA0\xE0-\xFE\x0A])
                (?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[^\x80-\xA0\xE0-\xFE])     (/s modifier)
  \d            [0-9]
...
functions.
  ...
  index       Char::Ehp15::index
  rindex      Char::Ehp15::rindex
  lc          Char::Ehp15::lc
  lcfirst     Char::Ehp15::lcfirst
  uc          Char::Ehp15::uc
  ucfirst     Char::Ehp15::ucfirst
  chr         Char::Ehp15::chr
  glob        Char::Ehp15::glob
  lstat       Char::Ehp15::lstat
  opendir     Char::Ehp15::opendir
  stat        Char::Ehp15::stat
  unlink      Char::Ehp15::unlink
  chdir       Char::Ehp15::chdir
  do          Char::Ehp15::do
  require     Char::Ehp15::require
  ---------------------------------
...
=item * Reverse List Or String
  ...
  %barfoo = Char::HP15::reverse(%foobar);
...
=item * Special Variables $` And $& Doesn't Function
  ...
  is escaped to
    'AAABBBCCC' =~ /\G(?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[^\x80-\xA0\xE0-\xFE])*?(?:BBB)@Char::Ehp15::m_matched/;

  For multibyte anchoring,
    <\G(?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[^\x80-\xA0\xE0-\xFE])*?> is added.
...
page 402 of Programming Perl, 3rd ed. again.
  ...
  Information processing model beginning with this software
    ...
    +-----------------------------------+
    |      ASCII Compatible Encoding    | ex. HP-15
    +-----------------------------------+
                (No UTF8 Flag)
