...
my $qq_substr  = qr{(?: HP15::substr | CORE::substr | substr ) \( $qq_paren \)
                 }xms;
...
my $function_getc;        # getc    to getc    or HP15::getc
...
END
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'HP15::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'HP15::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'HP15::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'HP15::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
# functions of package Ehp15
    ...
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc) { $slash = 'm//'; return "Ehp15::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }
    ...
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc) { $slash = 'm//'; return "Ehp15::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }
    ...
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\") ((?:$qq_char)+?)             (\") /oxgc)    { $slash = 'm//'; return "Ehp15::$1(" . e_qq('',  $2,$4,$3) . ")"; }
    ...
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc)    { $slash = 'm//'; return "Ehp15::$1(" . e_qq('qq',$2,$4,$3) . ")"; }

    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc)    { $slash = 'm//'; return "Ehp15::$1(" . e_q ('',  $2,$4,$3) . ")"; }
    ...
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc)    { $slash = 'm//'; return "Ehp15::$1(" . e_q ('q', $2,$4,$3) . ")"; }
...
# functions of package Ehp15
        ...
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc) { $e_string .= "Ehp15::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        ...
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc) { $e_string .= "Ehp15::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        ...
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\") ((?:$qq_char)+?)             (\") /oxgc)    { $e_string .= "Ehp15::$1(" . e_qq('',  $2,$4,$3) . ")"; $slash = 'm//'; }
        ...
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc)    { $e_string .= "Ehp15::$1(" . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }

        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc)    { $e_string .= "Ehp15::$1(" . e_q ('',  $2,$4,$3) . ")"; $slash = 'm//'; }
        ...
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc)    { $e_string .= "Ehp15::$1(" . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr with multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                                              2   3                                 4   5
                q<eval{local $HP15::re_t=%s; local $HP15::re_a=''; while($HP15::re_t =~ %s){%s local $^W=0; local $HP15::re_r=%s; %s$HP15::re_t="$HP15::re_a${1}$HP15::re_r$'"; pos($HP15::re_t)=length "$HP15::re_a${1}$HP15::re_r"; $HP15::re_a=substr($HP15::re_t,0,pos($HP15::re_t)); } return $HP15::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$HP15::re_r=eval $HP15::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $HP15::re_t=%s; while($HP15::re_t =~ %s){%s local $^W=0; local $HP15::re_r=%s; %s$HP15::re_t="$`$HP15::re_r$'"; pos($HP15::re_t)=length "$`$HP15::re_r"; } return $HP15::re_t}>,
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $HP15::re_r=%s; %s"%s$HP15::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g with multibyte anchoring
            ...
            $sub = sprintf(
                ...
                q<eval{local $HP15::re_n=0; local $HP15::re_a=''; while(%s =~ %s){%s local $^W=0; local $HP15::re_r=%s; %s%s="$HP15::re_a${1}$HP15::re_r$'"; pos(%s)=length "$HP15::re_a${1}$HP15::re_r"; $HP15::re_a=substr(%s,0,pos(%s)); $HP15::re_n++} return %s$HP15::re_n}>,
...
# (and so on)
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $HP15::re_n=0; while(%s =~ %s){%s local $^W=0; local $HP15::re_r=%s; %s%s="$`$HP15::re_r$'"; pos(%s)=length "$`$HP15::re_r"; $HP15::re_n++} return %s$HP15::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $HP15::re_r=%s; %s%s="%s$HP15::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $HP15::re_r=%s; %s%s="%s$HP15::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$HP15::re_r=eval $HP15::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
expand wildcard arguments supplied to programs. Instead, import of Ehp15.pm
works well.
...
Definitions in Ehp15.pm.
  ...
  ${Ehp15::eS}             qr{(?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[^\x80-\xA0\xE0-\xFE\s])}
  ...
  ${Ehp15::not_space}      qr{(?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[^\x80-\xA0\xE0-\xFE\s\x0B])}
...
=item * Substr by HP-15 Character
  ...
  If Perl version 5.14 or later, you can use the HP15::substr() function as an
