...
BEGIN {
    ...
    if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
        $PERL5LIB =~ s{[^\x80-\xA0\xE0-\xFE/]*$}{HP15};
    }
...
=item Make character
  ...
  set. For example, Ehp15::chr(65) is "A" in either ASCII or HP-15, not Unicode,
  and Ehp15::chr(0x82a0) is a HP-15 HIRAGANA LETTER A. For the reverse of
  Ehp15::chr, use HP15::ord.
...
=item File test operator -X
  ...
  Function and Prototype     Meaning
  ------------------------------------------------------------------------------
  Ehp15::s(*), Ehp15::s_()   File has nonzero size (returns size in bytes)
