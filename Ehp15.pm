...
${Ehp15::anchor}      = qr{\G(?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[^\x80-\xA0\xE0-\xFE])*?}oxms;
BEGIN { CORE::eval q{ use vars qw(
...
if (($] >= 5.010001) or
    ...
    # other encoding
        ...
        ${Ehp15::q_char_SADAHIRO_Tomoyuki_2002_01_17} = qr{.*?[$sbcs](?:$tbcs_1st[^$sbcs]{2})*?}oxms;
    }

    ${Ehp15::anchor_SADAHIRO_Tomoyuki_2002_01_17} =
    qr{\G(?(?=.{0,32766}\z)(?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[^\x80-\xA0\xE0-\xFE])*?|(?(?=[$sbcs]+\z).*?|(?:${Ehp15::q_char_SADAHIRO_Tomoyuki_2002_01_17})))}oxms;
#   qr{
#      \G # (1), (2)
#        (? # (3)
#          (?=.{0,32766}\z) # (4)
...
# ${Ehp15::eS}        = qr{(?:[\x80-\xA0\xE0-\xFE][\x00-\xFF]|[^\x80-\xA0\xE0-\xFE\x09\x0A\x0B\x0C\x0D\x20])};
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr with multibyte anchoring
            ...
            $sub = sprintf(
                #                              1                                              2   3                                 4   5
                q<CORE::eval{local $HP15::re_t=%s; local $HP15::re_a=''; while($HP15::re_t =~ %s){%s local $^W=0; local $HP15::re_r=%s; %s$HP15::re_t="$HP15::re_a${1}$HP15::re_r$'"; pos($HP15::re_t)=length "$HP15::re_a${1}$HP15::re_r"; $HP15::re_a=substr($HP15::re_t,0,pos($HP15::re_t)); } return $HP15::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$HP15::re_r=CORE::eval $HP15::re_r; ' x $e_modifier,            #  5
            );
        }
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                              1                        2   3                                 4   5
                q<CORE::eval{local $HP15::re_t=%s; while($HP15::re_t =~ %s){%s local $^W=0; local $HP15::re_r=%s; %s$HP15::re_t="$`$HP15::re_r$'"; pos($HP15::re_t)=length "$`$HP15::re_r"; } return $HP15::re_t}>,
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2                3                                 4   5  6                    7
                q<(%s =~ %s) ? CORE::eval{%s local $^W=0; local $HP15::re_r=%s; %s"%s$HP15::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g with multibyte anchoring
            ...
            $sub = sprintf(
                ...
                q<CORE::eval{local $HP15::re_n=0; local $HP15::re_a=''; while(%s =~ %s){%s local $^W=0; local $HP15::re_r=%s; %s%s="$HP15::re_a${1}$HP15::re_r$'"; pos(%s)=length "$HP15::re_a${1}$HP15::re_r"; $HP15::re_a=substr(%s,0,pos(%s)); $HP15::re_n++} return %s$HP15::re_n}>,
...
# (and so on)
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                       1     2   3                                 4   5 6                         7                                                 8
                q<CORE::eval{local $HP15::re_n=0; while(%s =~ %s){%s local $^W=0; local $HP15::re_r=%s; %s%s="$`$HP15::re_r$'"; pos(%s)=length "$`$HP15::re_r"; $HP15::re_n++} return %s$HP15::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3                4                                 5   6 7   8
                q<(%s%s%s) ? CORE::eval{%s local $^W=0; local $HP15::re_r=%s; %s%s="%s$HP15::re_r$'"; 1 } : undef> :

                #  1 2 3                    4                                 5   6 7   8
                q<(%s%s%s) ? 1 : CORE::eval{%s local $^W=0; local $HP15::re_r=%s; %s%s="%s$HP15::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$HP15::re_r=CORE::eval $HP15::re_r; ' x $e_modifier,            #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
