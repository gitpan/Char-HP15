...
BEGIN {
    ...
    if ($] < 5.006) {
        ...
        if ($^O ne 'MacOS') {
            *CORE::GLOBAL::binmode =
            *CORE::GLOBAL::binmode = \&Ehp15::binmode;
        }
        *CORE::GLOBAL::open    =
        *CORE::GLOBAL::open    = \&Ehp15::open;
    }
}
...
if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
    if ($ENV{'ComSpec'} =~ / (?: COMMAND\.COM | CMD\.EXE ) \z /oxmsi) {
        ...
        for (@ARGV) {
            ...
            elsif (m/\A (?:$q_char)*? [*?] /oxms and (my @glob = Ehp15::glob($_))) {
                push @argv, @glob;
            }
            else {
                push @argv, $_;
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $uc = Ehp15::uc($char)) ne $char) {
                $pattern .= $uc;
            }
            else {
...
This module is a run-time routines of the HP15.pm.
Because the HP15.pm automatically uses this module, you need not use directly.
...
=item binary mode (Perl5.6 emulation on perl5.005)
  ...
  Ehp15::binmode(FILEHANDLE, $disciplines);
  Ehp15::binmode(FILEHANDLE);
  Ehp15::binmode($filehandle, $disciplines);
  Ehp15::binmode($filehandle);
...
=item open file (Perl5.6 emulation on perl5.005)
  ...
  $rc = Ehp15::open(FILEHANDLE, $mode, $expr);
  $rc = Ehp15::open(FILEHANDLE, $expr);
  $rc = Ehp15::open(FILEHANDLE);
  $rc = Ehp15::open(my $filehandle, $mode, $expr);
  $rc = Ehp15::open(my $filehandle, $expr);
  $rc = Ehp15::open(my $filehandle);
  ...
  * three arguments
  ...
  {
      my $fh;                          # (uninitialized)
      Ehp15::open($fh, ">logfile")     # $fh is autovivified
  ...
  Ehp15::open my $fh, ">logfile" or die ...
  ...
  Ehp15::open(LOG, ">", "logfile") or die "Can't create logfile: $!";
  ...
  $path =~ s#^([ ])#./$1#;
  Ehp15::open (FH, "< $path\0") or die "can't open $path: $!";
  ...
  Ehp15::open(FH, "<", $path) or die "can't open $path: $!";
  ...
  Ehp15::open(FH, "<:raw", $path) or die "can't open $path: $!";
  ...
  Ehp15::open(FH, "<:crlf:encoding(HP15)", $path) or die "can't open $path: $!";
